...
function Get-AzTableTable
{
    ...
    if ($PSCmdlet.ParameterSetName -ne "AzStorageEmulator")
    {
        if ($PSCmdlet.ParameterSetName -eq "AzTableStorage" )
        {
            ...
            $nullTableErrorMessage = "Storage Table $TableName could not be retrieved from $storageAccountName on resource group $resourceGroup"
        ...
        else
        {
            ...
            $nullTableErrorMessage = "CosmosDB Table $TableName could not be retrieved from $cosmosDbAccountName on resource group $resourceGroup"
            $accountName = $cosmosDbAccountName
        }
        
        $keys = Invoke-AzResourceAction -Action listKeys -ResourceType $ResourceType -ResourceGroupName $resourceGroup -Name $accountName -Force -ErrorAction SilentlyContinue
